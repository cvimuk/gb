...
const App: React.FC = () => {
  ...
  const handleGeneratePrompts = async (foodList: string[], biteCount: number) => {
    if (!API_KEY) {
      alert("API Key not configured. Please set GEMINI_API_KEY in environment variables.");
      return;
    }
    ...
    await Promise.allSettled(newProjects.map(async (project) => {
      try {
        const result = await generatePromptsForFood(project.foodName, biteCount, API_KEY);
